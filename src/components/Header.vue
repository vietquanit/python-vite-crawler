        ...
        <div class="flex h-16 items-center justify-between">
          <div class="flex items-center">
            <div class="flex-shrink-0">
              <router-link to="/"><img class="h-8 w-8" src="./../assets/logo.png" alt="My logo" /></router-link>
            </div>
            <div class="hidden md:block">
              <div class="ml-10 flex items-baseline space-x-4">
                <router-link v-for="item in navigation" :key="item.name" :to="item.href" :class="'text-gray-300 hover:bg-gray-700 hover:text-white rounded-md px-3 py-2 text-sm font-medium'" >
                  {{ item.name }}
                </router-link>
...
const updateStateMenu = (item) => {
  item.current = true;
  navigation.forEach((value, index, array) => {
    if (value.href === item.href) {
      array[index] = item;
    } else {
      value.current = false;
    }
